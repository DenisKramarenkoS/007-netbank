...
function refreshBalance(localPersonObj) {
    infoValue.textContent = toIntlCurrency(localPersonObj.getCurrentBalance(), localPersonObj.region, localPersonObj.currency);
}
function refreshName(localPersonObj) {
    infoName.textContent = getShortName(localPersonObj.fullName) + '.';
}
// ---- Variables ----
function Person(fullName, password, initialBalance, interestRate, movements, movementsDates, movementsPerson, typeOfInvest, region, currency) {
...
const persons = [
    new Person('Gustavo Dior', 1111, 180_000, 4.5, [900, -40, 5000, -800], [
    ...
    ], ['John Knawel', 'Jane Claude Dame', 'Laura Poe', 'invest'], [null, null, null, 'Perspective'], 'it', 'EUR'),
    new Person('Aurora Sever', 2222, 9_909_999.93, 4.5, [9000, 904, -5000], [
        new Date('2024-09-24T14:18:46.330Z'),
        new Date('2024-10-12T07:45:23.294Z'),
        new Date('2025-01-04T04:20:53.194Z'),
    ], ['Henry Moro', 'Jovian Jayson', 'Alexander McQueen'], [null, null, null], 'en-US', 'USD'),
];
...
// ---- DOM Variables----
// Buttons
const btnAuth = document.getElementById('-btn__auth') || null;
const btnLogOut = document.getElementById('-btn__log-out') || null;
const btnSort = document.getElementById('-btn__sort') || null;
const btnTransfer = document.getElementById('-btn__transfer') || null;
const btnBorrow = document.getElementById('-btn__borrow') || null;
const btnInvest = document.getElementById('-btn__invest') || null;
// Input
const inputUsername = document.getElementById('-login__username') || null;
const inputPassword = document.getElementById('-login__password') || null;
const inputTransferValue = document.getElementById('-transfer__value') || null;
const inputTransferWhom = document.getElementById('-transfer__whom') || null;
const inputBorrowValue = document.getElementById('-borrow__value') || null;
const inputInvest = document.getElementById('-input__invest') || null;
// Divs
const mainContent = document.getElementById('-main-content') || null;
const nav = document.getElementById('-nav') || null;
const transferList = document.getElementById('-transfer__list') || null;
const infoName = document.getElementById('-info__name') || null;
const infoValue = document.getElementById('-info__value') || null;
const investBtnsOptions = document.getElementById('-invest__btns') || null;
// Classes
...
btnAuth.addEventListener('click', () => {
    ...
    personIndex = persons.findIndex((element) => getInitials(element.fullName) === inputUsername.value &&
    ...
    for (let i = 0; i < localPerson.movements.length; i++) {
        addTransfer(localPerson.movements[i], localPerson.movementsDates[i], localPerson.movementsPerson[i], localPerson.typeOfInvest[i], localPerson.region, localPerson.currency);
    }
...
btnTransfer.addEventListener('click', () => {
    ...
    addTransfer(localPerson.movements.at(-1), localPerson.movementsDates.at(-1), localPerson.movementsPerson.at(-1), null, localPerson.region, localPerson.currency);
    refreshBalance(localPerson);
});
...
btnBorrow.addEventListener('click', () => {
    ...
    addTransfer(localPerson.movements.at(-1), localPerson.movementsDates.at(-1), localPerson.movementsPerson.at(-1), localPerson.typeOfInvest.at(-1), localPerson.region, localPerson.currency);
    refreshBalance(localPerson);
});
investBtnsOptions.addEventListener('click', (event) => {
    ...
    if (target instanceof HTMLButtonElement) {
        ...
        investBtnCurrentText = target.textContent;
    }
});
...
transferList.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('btn__get-invest')) {
        const outerElement = target.parentElement;
        ...
        const percent = investMode === 'Not risky'
